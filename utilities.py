...
def sillburp_cooler(iter, z_index, flux, lat_range = None, sc=None,diff_val=31.536,temp_grad_base = 30/1000,
           file_path_dir='sillcubes/', post_cooling_time = 30000,saving_factor = [100], x=None,y=None,dx=None,dy=None, rock_prop_dict = None, save_dir_footnote = None):
    '''
    temp_grad_base == 30/1000 # Temperature gradient at the base of the modeled section (in C/m)
    k_val = Typical value of thermal diffusivity in the model
    ...
    a = int(y//dy) #Number of rows
    b = int(x//dx) #Number of columns
    props_array_vtk = pv.read(file_path_dir+'initial_sillburp_state_properties.vtk')
    props_array = props_array_vtk.point_data['data'].reshape(props_array_vtk.dimensions)
    props_array = np.array(props_array, dtype = object)
    props_array[sc.Temp_index] = np.array(props_array[sc.Temp_index], dtype = float)
    ...
    data = pv.read(file_path_dir+'initial_sillburp_state_carbon.vtk')
    
    if rock_prop_dict is None:
        rock_prop_dict = sc.rock_prop_dict

    RCO2 = data.point_data['RCO2'].reshape(data.dimensions[0], data.dimensions[1])
    RCO2 = RCO2.reshape(data.dimensions[0], data.dimensions[1])/props_array[sc.dense_index]
    Rom = data.point_data['Rom'].reshape(data.dimensions[0], data.dimensions[1])
    curr_TOC = data.point_data['curr_TOC'].reshape(data.dimensions[0], data.dimensions[1])

    progress_of_reactions = np.load(file_path_dir+'progress_of_reactions.npy')
    rate_of_reactions = np.load(file_path_dir+'rate_of_reactions.npy')
    
    if os.path.exists(file_path_dir+'oil.npy'):
        oil_production_rate = np.load(file_path_dir+'oil.npy')
    else:
        oil_df = pd.read_csv(file_path_dir+'oil.csv', index_col=0)
        oil_production_rate = np.array(oil_df)
        
    try:
        # Load and flatten the array
        sillburp_weights_df = pd.read_csv(file_path_dir + 'sillburp_weights.csv', index_col=0)
        sillburp_weights = np.array(sillburp_weights_df).flatten()
        
        # 1. Handle the empty case first using .size
        if sillburp_weights.size == 0:
            print('sillburp_weights is empty. Passing None...')
            sillburp_weights = None

        # 2. Check if it has exactly 4 elements to repeat them into a (4, a, b) shape
        elif sillburp_weights.size == 4:
            # Reshape the 4 elements to (4, 1, 1) so it can be stretched across 'a' and 'b' axes
            sillburp_weights = sillburp_weights.reshape(4, 1, 1)
            # Use tile to repeat the elements 'a' times on axis 1, and 'b' times on axis 2
            sillburp_weights = np.tile(sillburp_weights, (1, a, b))
        # 3. If it already matches the (4, a, b) shape requirement, reshape the flattened array back
        elif sillburp_weights.size == (4 * a * b):
            sillburp_weights = sillburp_weights.reshape(4, a, b)
            
        else:
            print(f"Warning: Unexpected array size {sillburp_weights.size}. Cannot format to (4, {a}, {b}).")
    ...
    tot_RCO2 = []
    carbon_model_params = [tot_RCO2, props_array, RCO2, Rom, progress_of_reactions, oil_production_rate, curr_TOC, rate_of_reactions, sillburp_weights]
    ...
    current_time = np.round(current_time, 3)
    carbon_model_params = sc.emplace_sills(props_array, n_sills, 'conv smooth', time_steps, current_time, sillsquare, carbon_model_params, empl_times, volume_params, z_index, saving_factor=saving_factor,model = 'sillburp', q=q, save_dir = dir_save)
    tot_RCO2 = carbon_model_params[0]
    timeframe['tot_RCO2'] = tot_RCO2
    timeframe['melt10'] = carbon_model_params[1][1:]
    timeframe['melt50'] = carbon_model_params[2][1:]
